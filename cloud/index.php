...
<head>
    ...
    <title>Cloud</title>
    <link rel="shortcut icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' fill='%23cc2455' class='bi bi-cloud-fill' viewBox='0 0 16 16'%3E%3Cpath d='M4.406 3.342A5.53 5.53 0 0 1 8 2c2.69 0 4.923 2 5.166 4.579C14.758 6.804 16 8.137 16 9.773 16 11.569 14.502 13 12.687 13H3.781C1.708 13 0 11.366 0 9.318c0-1.763 1.266-3.223 2.942-3.593.143-.863.698-1.723 1.464-2.383z'/%3E%3C/svg%3E" type="image/svg+xml" />
...
<body>
    ...
    <div class="content">
        <table>
            ...
            <?php
                exec("ls -lh /var/www/html/cloud/uploads",$output);  
                unset($output[0]);
                foreach($output as $value){
                    $array = preg_split('/ /', $value, -1, PREG_SPLIT_NO_EMPTY);
                    echo('<tr><td>'.$array[8].'</td><td>'.$array[4].'</td><td><a href="/cloud/uploads/'.$array[8].'" download title="Download"><svg class="download" width="30" height="40" xmlns="http://www.w3.org/2000/svg" fill="#cc2455" viewBox="0 0 16 16"><path d="M8 2a5.53 5.53 0 0 0-3.594 1.342c-.766.66-1.321 1.52-1.464 2.383C1.266 6.095 0 7.555 0 9.318 0 11.366 1.708 13 3.781 13h8.906C14.502 13 16 11.57 16 9.773c0-1.636-1.242-2.969-2.834-3.194C12.923 3.999 10.69 2 8 2zm2.354 6.854-2 2a.5.5 0 0 1-.708 0l-2-2a.5.5 0 1 1 .708-.708L7.5 9.293V5.5a.5.5 0 0 1 1 0v3.793l1.146-1.147a.5.5 0 0 1 .708.708z"></path></svg></a> <form action="/cloud/delete.php" method="post"> <input name="submit" type="submit" value="'.$array[8].'" style="display: none;"/> <label title="Delete" class="svg-d" onclick="f_delete(this)"> <svg xmlns="http://www.w3.org/2000/svg" width="25" height="40" fill="#cc2455" class="bi bi-trash-fill" viewBox="0 0 16 16"> <path d="M2.5 1a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1H3v9a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V4h.5a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1H2.5zm3 4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 .5-.5zM8 5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7A.5.5 0 0 1 8 5zm3 .5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 1 0z"/> </svg> </label> </form> <form action="/cloud/rename.php" method="post"> <input class="submit-r" type="submit" value="'.$array[8].'|file-rename|" name="submit" style="display: none;"/><label class="svg-r" title="Rename" onclick="f_rename(this)"><svg xmlns="http://www.w3.org/2000/svg" style="width: 30px; height: 40px;vertical-align: middle;fill: #cc2455;overflow: hidden;" viewBox="0 0 1024 1024" version="1.1"><path d="M544 768l-384 0C142.336 768 128 753.664 128 736l0-384C128 334.336 142.336 320 160 320l384 0C561.664 320 576 334.336 576 352S561.664 384 544 384L192 384l0 320 352 0C561.664 704 576 718.336 576 736S561.664 768 544 768zM288 512C270.336 512 256 526.336 256 544S270.336 576 288 576l192 0C497.664 576 512 561.664 512 544S497.664 512 480 512L288 512zM800 896 704 896 704 192l96 0C817.664 192 832 177.664 832 160S817.664 128 800 128l-256 0C526.336 128 512 142.336 512 160S526.336 192 544 192L640 192l0 704L544 896C526.336 896 512 910.336 512 928S526.336 960 544 960l256 0c17.664 0 32-14.336 32-32S817.664 896 800 896zM864 320l-64 0C782.336 320 768 334.336 768 352S782.336 384 800 384L832 384l0 320-32 0c-17.664 0-32 14.336-32 32s14.336 32 32 32l64 0c17.664 0 32-14.336 32-32l0-384C896 334.336 881.664 320 864 320z"/></svg></label></form></td></tr>');
                }
            ?>
    ...
    <div class="controls">
        <form action="/cloud/upload.php" method="POST" enctype="multipart/form-data">
            <input type="file" id="file" name="files" style="display: none;" onchange="change()"/>
            <label for="file">
                <div class="upload">
                    ...
                    <div class="text--" style="margin-top: -42px; margin-left: 54px;">Upload</div>
                <div>
            </label>
            <input type="submit" name="submit" value="file.name" style="display: none;" id="submit"/>
        </form>
        <script>
            document.getElementById('file').value = '';
            function change() {
                if(confirm("Upload "+document.getElementById('file').value.split('\\')[document.getElementById('file').value.split('\\').length-1]+"\nPress Ok To Upload.")) {
                    document.getElementById('submit').click();
                } else {
                    document.getElementById('file').value = '';
                }
            }
            function f_delete(elm) {
                if(confirm("Delete "+elm.parentElement.children[0].value+"\nPress Ok To Delete Permanently.")) {
                    elm.parentElement.children[0].click();
                }
            }
            function f_rename(elm) {
                let new_name = prompt("Rename "+elm.parentElement.children[0].value.split('|file-rename|')[0]+"\nEnter New File Name.")
                if(new_name!=null&&new_name.split('|file-rename|')[0]!=''&&new_name.split('/')[0]==new_name&&new_name.split('|file-rename|')[0]!=elm.parentElement.children[0].value.split('|file-rename|')[0]) {
                    elm.parentElement.children[0].value += new_name.split('|file-rename|')[0];
                    elm.parentElement.children[0].click();
                } else {
                    alert('Sorry Invalid Input or Cancelled By User.');
                }
            }
        </script>
    </div>
